MODULE strings
! Author: Dr. George Benthien
! http://www.gbenthien.net/strings/index.html. Last accessed on Jan 1st, 2012

use shr_kind_mod  , only : kr8 => shr_kind_r8, kr4 => shr_kind_r4, ki8=>SHR_KIND_I8, ki4=>SHR_KIND_I4
...
interface str2num  ! Generic operator for converting a number string to a 
                 ! number. Calling syntax is 'call str2num(numstring,number,ios)' 
...
interface num2str  ! Generic  interface for writing a number to a string. The 
                    ! number is left justified in the string. The calling syntax
                    ! is 'call num2str(number,string,format)' where 'number' is
...
call num2str(value,tempstr,fmt)
...
character(len=*) :: namestr,fmt
character(len=32) :: tempstr
call num2str(ivalue,tempstr,fmt)
call trimzero(tempstr)
write(unit,*) trim(namestr)//' = '//trim(tempstr)
